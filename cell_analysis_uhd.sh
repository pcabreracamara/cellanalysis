...
# Variables
KALBIN="/opt/kalibrate-uhd/src/kal"
...
if [ ! -f "${GSMRECPATH}gsm_receive_usrp.py" ]
then
        echo "Exit !!, ${GSMRECPATH}gsm_receive_usrp.py no existe."
        exit 0
fi
...
then
	...
	do
		arfcn=`echo "${linea}" | awk '{print $(NF-5)}'`
		freq_tmp=`echo "${linea}" | awk '{print $(NF-4)}' | sed 's/(//g'| sed 's/MHz//g' | sed 's/\.//g'`
		freq_base=`echo ${freq_tmp}00000`
		signo=`echo "${linea}" | awk '{print $(NF-3)}'`
		offset=`echo "${linea}" | awk '{print $(NF-2)}' | sed 's/)//g'| sed 's/kHz//g' | sed 's/\.//g'`
		...
		./gsm_receive_usrp.py -f ${freq_final} -d 174 -c 0B -g 40  > /dev/null 2>&1 &
		sleep 20
		disown
		kill -9 $! > /dev/null
		cd - > /dev/null 2>&1
